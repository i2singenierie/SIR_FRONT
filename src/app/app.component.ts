...
fond_carte = new LayerGroup({
  title: "Base maps",
  layers: [this.osm,this.google, this.watercolor,this.thunderforest],
...
// fin -- pour layer switcher
   ...
   // map.addLayer(this.MAR2);    // couche vecteur
    ...
    this.source_orcl_json.on('addfeature', () =>{
         map.getView().fit(
          ...
          { duration: 1000, size: map.getSize(), maxZoom: 15 }
      );
  });
 ...
 // add 
    ...
    // add control 2
     //button
     var localisation_sect = document.createElement('button');
     localisation_sect.innerHTML = '<i class="bi bi-rulers red-color"></i>';
     localisation_sect.className = 'myButton_sect';
     localisation_sect.id = 'localisation_sect';

     var localisationElement_sect = document.createElement('div');
     localisationElement_sect.className = 'localisationDiv_sect';
     localisationElement_sect.appendChild(localisation_sect);

        var control2 = new Control({
      element: localisationElement_sect
  })
// controle 3
  ...
  localisationElement_section.appendChild(sect);
   ...
   element: localisationElement_section
     ...
     localisation_sect.addEventListener("click",()=>{
      localisation_sect.classList.toggle('clicked');
      this.verificationClick_sect = this.verificationClick_sect?false:true;
     })
 ...
 if(this.verificationClick){
  ...
  //todo1
    ...
    (res)=>{
      ...
      this.display_point_properties(res[0].ROUTE_NAME,res[0].PKD)
    },
    (err)=>{
      console.log(err);
    }
...
// pour les sections entre 2 points
  ...
  display_line_properties(route_name:any,pkd:any,pkf:any){
    var contentTable = '<table id="customers"> <tr> <th>ROUTE NAME</th> <th>PKD</th> <th>PKF</th> </tr> <tr>';
          contentTable = contentTable + `<td>${route_name}</td>`;
          contentTable = contentTable + `<td>${pkd/1000}</td>`;
          contentTable = contentTable + `<td>${pkf/1000}</td>`;
          contentTable = contentTable + "<tr> </table>";
          if(this.content) this.content.innerHTML = contentTable;
